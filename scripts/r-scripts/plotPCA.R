...
# ===== LOAD META DATA =====
meta_data <- read_delim("doc/ischnura_metadata.tsv", delim = "\t", col_names = T)
...
pc12 <- pca %>%
  ggplot(aes(PC1, PC2, fill = Population, shape = Sex)) +
...
pc34 <- pca %>%
  ggplot(aes(PC3, PC4, fill = Population, shape = Sex)) +
  geom_point(size = 4, stroke = 0.8, colour = "black") +
  scale_shape_manual(values = c(21, 24)) +
  scale_fill_brewer(palette = "Set2") +
  geom_hline(yintercept=0, linetype="dashed", color = "gray", alpha = 0.5) +
...
# PC3 vs PC4 (filled shapes)
pc56 <- pca %>%
  ggplot(aes(PC5, PC6, fill = Population, shape = Sex)) +
  geom_point(size = 4, stroke = 0.8, colour = "black") +
  scale_shape_manual(values = c(21, 24)) +
  scale_fill_brewer(palette = "Set2") +
  geom_hline(yintercept=0, linetype="dashed", color = "gray", alpha = 0.5) +
